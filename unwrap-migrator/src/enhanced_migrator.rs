...
impl EnhancedUnwrapMigrator {
    ...
    pub async fn migrate_crate(&self, crate_path: &Path) -> Result<BatchMigrationResults, MigrationError> {
    ...
    async fn migrate_single_file(&self, file_path: &Path) -> Result<SingleFileMigrationResults, MigrationError> {
    ...
    fn discover_rust_files_sync(&self, path: &Path, files: &mut Vec<PathBuf>) -> Result<(), MigrationError> {
    ...
    pub async fn generate_report(&self, root_path: &Path) -> Result<MigrationReport, MigrationError> {
        ...
        report.file_statistics = sorted_files.into_iter()
            .map(|(k, v)| (k.clone(), *v))
            .collect();
    ...
    async fn apply_fixes_to_file(&self, file_path: &Path, fixes: &[UnwrapFix]) -> Result<(), MigrationError> {
        ...
        tokio::fs::write(file_path, modified_content).await.map_err(|e| {
            MigrationError::IoError(format!("Failed to write file {}: {}", file_path.display(), e))
        })?;
    ...
    fn apply_fix_to_line(&self, line: &str, fix: &UnwrapFix) -> String {
        ...
        match &fix.fix_type {
            ...
            FixType::ReplaceExpected { original_message: _ } => {
                let expect_regex = regex::Regex::new(r#"\.expect\("[^"]*"\)"#).unwrap();
                if is_test_context {
            ...
            FixType::ReplacePanic { original_message: _ } => {
    ...
    async fn collect_rust_files(&self, root_path: &Path, entries: &mut Vec<PathBuf>) -> Result<(), MigrationError> {
        let mut dir_entries = tokio::fs::read_dir(root_path).await.map_err(|e| {
            MigrationError::IoError(format!("Failed to read directory {}: {}", root_path.display(), e))
        })?;
...
mod tests {
    ...
    async fn test_context_categorization() {
        ...
        let env_context = "let port = std::env::var(\"PORT\").map_err(|e| {
    ...
    std::io::Error::new(std::io::ErrorKind::NotFound, format!(\"Environment variable error: {}\", e))
        ...
        assert!(migrator.category_mappings.get("env::var").map_or(false, |cat| cat.contains("Configuration")));
    ...
    std::io::Error::new(std::io::ErrorKind::InvalidData, format!(\"JSON parsing error: {}\", e))
        ...
        assert!(migrator.category_mappings.get("serde_json").map_or(false, |cat| cat.contains("Validation")));
    }
    
    #[tokio::test]
    async fn test_file_discovery() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(rust_files.iter().any(|f| f.file_name().and_then(|n| n.to_str()) == Some("test.rs")));
        assert!(rust_files.iter().any(|f| f.file_name().and_then(|n| n.to_str()) == Some("lib.rs")));
