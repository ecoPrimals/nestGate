...
See the upstream [wateringHole repository](https://git.primals.eco/ecoPrimals/wateringHole) `SCYBORG_PROVENANCE_TRIO_GUIDANCE.md` for the full standard.
...
- Rust and other **program source** under `code/`, `src/`, `tests/**/*.rs`, and `benches/` (source and build scripts)
...
- **Markdown and prose documentation** outside of executable source: e.g. root `*.md` (including this file), `docs/**`, `capability_registry.toml`, `examples/**/*.md`, `tests/**/*.md`, and `README.md` / guide files under `code/crates/**` when they are documentation, not code
