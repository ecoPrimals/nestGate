...
impl ConfigMigrator {
    ...
    pub fn rollback(&self) -> Result<()> {
        if let Some(backup) = &self.backup {
            ...
            Err(NestGateError::Configuration {
                ...
                current_value: None,
                expected: Some("Valid backup".to_string()),
                user_error: false,
            })
        }
    }
    ...
    fn get_backup_path(&self) -> Result<PathBuf> {
        if let Some(backup_dir) = &self.options.backup_directory {
            Ok(backup_dir.join(format!("config_backup_{}.json", 
                SystemTime::now().duration_since(SystemTime::UNIX_EPOCH)
                    .unwrap_or_default().as_secs())))
        } else {
            Ok(PathBuf::from(format!("config_backup_{}.json", 
                SystemTime::now().duration_since(SystemTime::UNIX_EPOCH)
                    .unwrap_or_default().as_secs())))
        }
    }
...
impl MigrationReport {
    ...
    pub fn is_successful(&self) -> bool {
        ...
        self.failed_steps.iter().all(|e| !matches!(e.severity, ErrorSeverity::Critical))
    }
...
/// Validation rule for configuration migration
#[derive(Debug, Clone)]
pub struct ValidationRule {
    ...
    /// Validation function
    pub validator: fn(&NestGateCanonicalConfig) -> Result<()>,
}
...
impl SafeConfigMigration {
    ...
    pub fn migrate_with_backup<T, U>(
        &mut self,
        from: T,
        to: U
    ) -> Result<U>
    ...
    fn validate_value_ranges(_config: &NestGateCanonicalConfig) -> Result<()> {
