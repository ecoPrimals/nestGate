...
pub struct ConsolidatedIntegrationConfigs {
    ...
    /// Ecosystem integrations (BiomeOS, etc.)
...
pub struct EcosystemConfig {
    ...
    pub biomeos: BiomeOsIntegrationConfig,
    ...
    fn validate_for_environment(&self, env: &str) -> Result<()> {
        ...
        match env {
            "production" => {
                ...
                if self.api.server.port == 8080 {
                    return Err(crate::error::NestGateError::Configuration {
                        field: "api.server.port".to_string(),
                        message: "Port 8080 not recommended for production".to_string(),
                        current_value: Some("8080".to_string()),
                        expected: Some("443 or custom port".to_string()),
                        user_error: true,
                    });
                }
            },
...
pub mod migration {
    ...
    fn migrate_zfs_config(legacy: &serde_json::Value) -> Result<ZfsDomainConfig> {
        // Implementation for ZFS config migration
        Ok(ZfsDomainConfig::default())
    }
    
    fn migrate_api_config(legacy: &serde_json::Value) -> Result<ApiDomainConfig> {
...
impl DomainConfigValidation for ZfsDomainConfig {
    fn validate(&self) -> Result<Vec<String>> { Ok(vec![]) }
    fn validate_for_environment(&self, _env: &str) -> Result<()> { Ok(()) }
    fn required_fields() -> Vec<&'static str> { vec![] }
    fn optional_fields() -> Vec<&'static str> { vec![] }
}
