...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EncryptionConfig { pub enabled: bool }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct KeyManagementStorageConfig { pub provider: String }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DataEncryptionConfig { pub enabled: bool }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TransitEncryptionConfig { pub enabled: bool }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EncryptionAlgorithmStorageConfig { pub algorithm: String }

impl Default for StorageEncryptionConfig {
    fn default() -> Self {
        Self {
            encryption: EncryptionConfig { enabled: false },
            key_management: KeyManagementStorageConfig { provider: "local".to_string() },
            data_encryption: DataEncryptionConfig { enabled: false },
            transit_encryption: TransitEncryptionConfig { enabled: true },
            algorithm: EncryptionAlgorithmStorageConfig { algorithm: "AES-256-GCM".to_string() },
        }
    }
}
