...
impl StorageBackendConfig {
    ...
    pub fn validate(&self) -> crate::Result<()> {
        ...
        if !self.backends.is_empty() && !self.backends.contains_key(&format!("{:?}", self.default_backend).to_lowercase()) {
        ...
        for rule in &self.routing.rules {
            if !self.backends.contains_key(&rule.backend) {
                return Err(crate::NestGateError::validation_error(
                    format!("Routing rule '{}' references non-existent backend '{}'", rule.name, rule.backend)
                ));
            }
        }
    ...
    pub fn get_available_backends(&self) -> Vec<StorageBackendType> {
        self.backends.values().map(|b| b.backend_type.clone()).collect()
    }

    /// Check if a specific backend is configured
    pub fn has_backend(&self, backend_type: &StorageBackendType) -> bool {
        self.backends.values().any(|b| &b.backend_type == backend_type)
    }
} 