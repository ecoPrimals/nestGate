...
//!
//! CPU optimization and scheduling configuration.
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.thread_pools.core_size == 0 {
            return Err(NestGateError::Configuration {
                field: "cpu.thread_pools.core_size".to_string(),
                message: "Core thread pool size cannot be zero".to_string(),
        ...
        if self.thread_pools.max_size < self.thread_pools.core_size {
            return Err(NestGateError::Configuration {
                field: "cpu.thread_pools.max_size".to_string(),
                message: "Maximum thread pool size cannot be less than core size".to_string(),
                current_value: Some(self.thread_pools.max_size.to_string()),
                expected: Some(format!(">={}", self.thread_pools.core_size)),
                user_error: true,
            });
        }
        ...
        if self.monitoring.usage_threshold < 0.0 || self.monitoring.usage_threshold > 1.0 {
            return Err(NestGateError::Configuration {
                field: "cpu.monitoring.usage_threshold".to_string(),
                message: "CPU usage threshold must be between 0.0 and 1.0".to_string(),
                current_value: Some(self.monitoring.usage_threshold.to_string()),
