...
1. **Ecosystem Agnostic**: No hardcoded dependencies on specific primals
2. **Capability-Based**: Dynamic feature negotiation rather than fixed interfaces
3. **Auto-Discovery**: Automatic detection and integration of available primals
4. **Future-Proof**: Extensible architecture for unknown future primal types
5. **Security-First**: Mutual TLS, encryption, and audit logging by default

### Core Components

```mermaid
graph TD
    A[NestGate Universal Storage Primal] --> B[Universal Primal Interface]
    B --> C[Auto-Discovery Engine]
    B --> D[Capability Negotiator]
    B --> E[Security Manager]
    B --> F[Configuration Manager]
    
    C --> G[Network Scanner]
    C --> H[Environment Reader]
    C --> I[Config Loader]
    C --> J[Service Registry]
    
    D --> K[Capability Cache]
    D --> L[Version Resolver]
    D --> M[Conflict Resolver]
    
    E --> N[Mutual TLS]
    E --> O[Encryption Engine]
    E --> P[Audit Logger]
    
    F --> Q[TOML Config]
    F --> R[Environment Override]
    F --> S[Runtime Updates]
```

## Universal Primal Interface

### StoragePrimalProvider Trait

```rust
#[async_trait]
pub trait StoragePrimalProvider {
    /// Handle universal primal requests
    async fn handle_request(&self, request: StoragePrimalRequest) -> Result<StoragePrimalResponse>;
    
    /// Stream data for large operations
    ...
    async fn handle_request(&self, request: StoragePrimalRequest) -> Result<StoragePrimalResponse> {
        match request {
            ...
            StoragePrimalRequest::AiDataRequest { request } => {
                // Delegate to AI primal if available
                self.delegate_to_ai_primal(request).await
            },
