...
### **biomeOS Universal Adapter Integration Flow**
```
biome.yaml (Universal Genome) → Universal Adapter → Capability Discovery → Unknown Ecosystem Participants
                                        ↓
                             Network Effects Engine → Capability Composition + Value Multiplication
```

### **Communication Pattern: "Universal Adapter Pattern"**
- **Capability-based discovery** automatically finds available capabilities (provider-agnostic)
- **Universal adapter coordination** - single integration point, zero hardcoded service names
- **Dynamic capability routing** based on detected capabilities, not service identities
- **Network effects emergence** through capability composition across unknown participants
