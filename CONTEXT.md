...
| **TLS** | No OpenSSL; **rustls** only |
| **Unsafe** | None in normal crates; **env-process-shim** bridge is the documented exception |
| **Lint / format** | Workspace `clippy` with `-D warnings` (pedantic + nursery); `cargo fmt --check` clean |
| **Tests** | 7,887 library tests passing, 0 failures |
| **Coverage** | 74.5% line (llvm-cov) |
