...
pub async fn demo_individual_user_experience() -> Result<()> {
    ...
    // Create spore for any primal (e.g., "songbird", "toadstool", "squirrel")
...
/// Demo: BearDog Integration (Enhanced Capabilities)
pub async fn demo_beardog_integration() -> Result<()> {
    ...
    let beardog_endpoint = Some("https://beardog.local:8443".to_string());
    spore_standalone.initialize_with_beardog(beardog_endpoint).await?;
    
    println!("🔗 After BearDog integration:");
    println!("   - BearDog integrated: {}", spore_standalone.beardog_integration.is_some());
...
pub async fn demo_universal_integration_pattern() -> Result<()> {
    ...
    // Demonstrate the same pattern works for different primals
    let primals = vec!["songbird", "toadstool", "squirrel", "nestgate", "beardog"];
    
    for primal_name in primals {
        let spore = UniversalCryptographicSpore::new_for_primal(primal_name)?;
        println!("✅ {} spore created: {}", primal_name, spore.spore_id);
