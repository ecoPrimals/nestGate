...
use nestgate_core::universal_model_api::{
    AuthenticationRequirements, AuthenticationType, InferenceInput, InferenceRequest,
    ModelCapability, ModelConfiguration, ModelHandle, ModelInfo, ModelLoadRequest, ModelParameters,
    ModelPrecision, ModelType, OptimizationLevel, OptimizationSettings, ProviderConfig,
    RegistryConfig, ResourceRequirements, UniversalModelProviderFactory, UniversalModelRegistry,
};
use serde_json::json;
...
async fn register_multiple_providers(registry: &Arc<UniversalModelRegistry>) -> Result<()> {
    ...
    let local_config = ProviderConfig {
        ...
        api_key: None,
        organization: None,
        model_cache_path: Some("/models".to_string()),
...
fn example_configurations() -> Vec<ProviderConfig> {
    vec![
        ...
        ProviderConfig {
            ...
            organization: Some("org-your_org_id".to_string()),
            model_cache_path: None,
            timeout_seconds: Some(60),
            max_retries: Some(3),
            custom_headers: None,
