...
mod tests {
    ...
    fn test_builder_pattern() {
        let config = ProductionDiscoveryConfig::new()
            .with_service_host("API".into(), "api.example.com".into())
            .with_service_port("API".into(), 8080)
            .with_service_bind("API".into(), "0.0.0.0".into())
            .with_resource_limit("max_connections".into(), 1000)
            .with_operation_timeout("connect".into(), 30);
    ...
    fn test_getters() {
        ...
        config
            .service_hosts
            .insert("WEB".into(), "web.example.com".into());
        config.service_ports.insert("WEB".into(), 3000);
    ...
    async fn test_concurrent_different_configs() {
        let config1 = Arc::new(
            ProductionDiscoveryConfig::new()
                .with_service_host("API".into(), "api1.example.com".into())
                .with_service_port("API".into(), 8080),
        );
        let config2 = Arc::new(
            ProductionDiscoveryConfig::new()
                .with_service_host("API".into(), "api2.example.com".into())
                .with_service_port("API".into(), 9090),
        );
        ...
        assert_eq!(host1, Some("api1.example.com".into()));
        assert_eq!(port1, Some(8080));
        assert_eq!(host2, Some("api2.example.com".into()));
