...
impl DummyClient {
    ...
    async fn mount_volume(&self, _request: McpMountRequest) -> Result<McpMountResponse, anyhow::Error> {
        // This would make an actual API call
        Ok(McpMountResponse {
    ...
    async fn unmount_volume(&self, _request: McpUnmountRequest) -> Result<McpUnmountResponse, anyhow::Error> {
        // This would make an actual API call
    ...
    async fn get_metrics(&self, _detailed: bool, _range: u64) -> Result<MetricsResponse, Box<dyn std::error::Error>> {
        // This would make an actual API call
        Ok(MetricsResponse {
    ...
    async fn get_storage_health(&self) -> Result<StorageHealthResponse, Box<dyn std::error::Error>> {
        // This would make an actual API call
    ...
    async fn create_snapshot(&self, volume_id: &str, name: &str) -> Result<SnapshotResponse, Box<dyn std::error::Error>> {
        // This would make an actual API call
        Ok(SnapshotResponse {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config_path = cli.config.unwrap_or_else(|| {
        PathBuf::from(env::var("NESTGATE_CONFIG").unwrap_or_else(|_| "/etc/nestgate/config.toml".to_string()))
    });
...
async fn mount_volume(client: &NestGateClient, args: MountArgs) -> Result<(), Box<dyn std::error::Error>> {
...
async fn unmount_volume(client: &NestGateClient, args: UnmountArgs) -> Result<(), Box<dyn std::error::Error>> {
    // Create request
...
async fn show_metrics(client: &NestGateClient, args: MetricsArgs) -> Result<(), Box<dyn std::error::Error>> {
    // Get metrics
    ...
    match args.format.to_lowercase().as_str() {
        ...
        _ => {
            ...
            println!("--------------------------");
            println!("Timestamp: {}", metrics.timestamp.format("%Y-%m-%d %H:%M:%S"));
...
async fn manage_storage(client: &NestGateClient, args: StorageArgs) -> Result<(), Box<dyn std::error::Error>> {
    match args.command {
        ...
        StorageCommands::Health => {
            ...
            println!("---------------------");
            println!("Timestamp: {}", health.timestamp.format("%Y-%m-%d %H:%M:%S"));
        ...
        StorageCommands::Snapshot { volume_id, name } => {
            ...
            println!("Created: {}", snapshot.timestamp.format("%Y-%m-%d %H:%M:%S"));
