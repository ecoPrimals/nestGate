...
impl ServiceManager {
    ...
    pub async fn execute(&mut self, action: ServiceAction) -> BinResult<(), NestGateUnifiedError> {
        match action {
            ServiceAction::Start { port, config } => {
                self.start_service(port, config.as_deref()).await
            }
            ServiceAction::Stop => {
                self.stop_service().await
            }
            ServiceAction::Restart { port, config } => {
                self.restart_service(port, config.as_deref()).await
            }
    ...
    async fn start_service(&self, port: Option<u16>, config: Option<&str>) -> BinResult<(), NestGateUnifiedError> {
        ...
        println!("🌐 API available at: http://{}:{}", nestgate_core::constants::network::LOCALHOST, port);
        println!("🔍 Health check: http://{}:{}/health", nestgate_core::constants::network::LOCALHOST, port);
        
        Ok(())
    }

    // Stop NestGate service
    async fn stop_service(&self) -> BinResult<(), NestGateUnifiedError> {
    ...
    async fn restart_service(&self, port: Option<u16>, config: Option<&str>) -> BinResult<(), NestGateUnifiedError> {
    ...
    // Show service status
    async fn show_status(&self) -> BinResult<(), NestGateUnifiedError> {
        ...
        println!("  Port: nestgate_core::constants::network::DEFAULT_API_PORT"); // Would be from actual service
