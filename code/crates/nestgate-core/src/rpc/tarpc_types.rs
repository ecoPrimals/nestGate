...
pub trait NestGateRpc {
    ...
    async fn create_dataset(name: String, params: DatasetParams) -> Result<DatasetInfo, NestGateRpcError>;
    ...
    async fn get_object_metadata(dataset: String, key: String) -> Result<ObjectInfo, NestGateRpcError>;
    ...
    /// # Returns
    /// Success result
    async fn delete_object(dataset: String, key: String) -> Result<OperationResult, NestGateRpcError>;
    ...
    /// # Returns
    /// Registration result
    async fn register_capability(registration: CapabilityRegistration) -> Result<RegistrationResult, NestGateRpcError>;
...
pub enum NestGateRpcError {
    ...
    /// Quota exceeded
    QuotaExceeded { dataset: String, quota: u64, requested: u64 },
...
impl std::fmt::Display for NestGateRpcError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::DatasetAlreadyExists { dataset } => write!(f, "Dataset already exists: {}", dataset),
            Self::ObjectNotFound { dataset, key } => write!(f, "Object not found: {}/{}", dataset, key),
            Self::ObjectAlreadyExists { dataset, key } => write!(f, "Object already exists: {}/{}", dataset, key),
            Self::InvalidParameters { message } => write!(f, "Invalid parameters: {}", message),
            Self::StorageFull { required, available } => write!(f, "Storage full: required {} bytes, available {} bytes", required, available),
            Self::QuotaExceeded { dataset, quota, requested } => write!(f, "Quota exceeded for dataset {}: quota {} bytes, requested {} bytes", dataset, quota, requested),
