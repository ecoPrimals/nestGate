...
/// Configuration for VendorAgnostic
pub struct VendorAgnosticConfig {
...
impl Default for VendorAgnosticConfig {
...
pub struct DefaultService {
    config: VendorAgnosticConfig,
    metrics: Arc<tokio::sync::RwLock<Metrics>>,
}

impl DefaultService {
    /// Create a new service instance
    pub fn new(config: VendorAgnosticConfig) -> Self {
        Self {
            config,
...
impl Service for DefaultService {
    ...
    fn initialize(&self) -> impl std::future::Future<Output = Result<()>> + Send {
        ...
        tracing::info!("Initializing {} service with config: {:?}", 
                      stringify!(vendor_agnostic), config);
        Ok(())
    }
    ...
    fn shutdown(&self) -> impl std::future::Future<Output = Result<()>> + Send {
        ...
        tracing::info!("Shutting down {} service", stringify!(vendor_agnostic));
        Ok(())
    }
}
...
pub fn create_service() -> DefaultService {
    DefaultService::new(VendorAgnosticConfig::default())
}

/// Validate configuration
pub async fn validate_config(config: &VendorAgnosticConfig) -> crate::Result<()> {
    if config.max_connections == 0 {
        return Err(NestGateError::configuration_error(
            "config_vendor_agnostic",
...
mod tests {
    ...
    fn test_config_default() {
        let config = VendorAgnosticConfig::default();
