...
use super::{
    NestGateCanonicalUnifiedConfig, ApiConfig, PerformanceHandlerConfig,
};
    ...
    pub fn resolve_type_alias_config(
        ...
        match alias_name {
            "UnifiedApiHandlerConfig" => {
                ...
                let fragmented: FragmentedApiHandlerConfig = serde_json::from_value(source_data.clone())
                    .map_err(|e| NestGateError::Configuration {
                        message: format!("Failed to parse fragmented API handler config: {e}"),
            ...
            _ => {
                self.add_warning(
                    MigrationWarningCategory::ConfigIgnored,
                    format!("Unknown type alias: {alias_name}"),
                    Some(alias_name.to_string()),
...
impl ConfigMigrationManager {
    ...
    pub fn migrate_from_json_file(&mut self, file_path: &str) -> Result<NestGateCanonicalUnifiedConfig> {
        let content = std::fs::read_to_string(file_path)
            .map_err(|e| NestGateError::Configuration {
                message: format!("Failed to read config file {file_path}: {e}"),
        ...
        self.migrate_from_json_value(&json_value)
    }

    /// Migrate from JSON value
    pub fn migrate_from_json_value(&mut self, json_value: &serde_json::Value) -> Result<NestGateCanonicalUnifiedConfig> {
        // Try to parse as fragmented API handler config first
        if let Ok(fragmented) = serde_json::from_value::<FragmentedApiHandlerConfig>(json_value.clone()) {
            let mut canonical_config = NestGateCanonicalUnifiedConfig::default();
            canonical_config.api = self.migrate_api_handler_config(&fragmented)?;
        ...
        Ok(NestGateCanonicalUnifiedConfig::default())
    }
} 