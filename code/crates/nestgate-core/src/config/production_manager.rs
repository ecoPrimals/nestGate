...
//! Modern production_manager Module
//! 
//! This module provides configuration functionality using modern Rust patterns
...
/// ```rust
/// // OLD (deprecated):
/// use crate::config::ProductionManagerConfig;
/// 
/// // NEW (canonical):
/// use crate::config::canonical_primary::domains::network::CanonicalNetworkConfig;
/// // Or use type alias for compatibility:
/// use crate::config::ProductionManagerConfig; // Now aliases to CanonicalNetworkConfig
/// ```
/// 
...
/// Configuration for ProductionManager
pub struct ProductionManagerConfig {
...
impl Default for ProductionManagerConfig {
...
pub struct DefaultService {
    config: ProductionManagerConfig,
    metrics: Arc<tokio::sync::RwLock<Metrics>>,
}

impl DefaultService {
    /// Create a new service instance
    pub fn new(config: ProductionManagerConfig) -> Self {
        Self {
            config,
...
impl Service for DefaultService {
    ...
    fn initialize(&self) -> impl std::future::Future<Output = Result<()>> + Send {
        ...
        tracing::info!("Initializing {} service with config: {:?}", 
                      stringify!(production_manager), config);
        Ok(())
    }
    ...
    fn shutdown(&self) -> impl std::future::Future<Output = Result<()>> + Send {
        ...
        tracing::info!("Shutting down {} service", stringify!(production_manager));
        Ok(())
    }
}
...
pub fn create_service() -> DefaultService {
    DefaultService::new(ProductionManagerConfig::default())
}

/// Validate configuration
pub async fn validate_config(config: &ProductionManagerConfig) -> crate::Result<()> {
    if config.max_connections == 0 {
        return Err(NestGateError::configuration_error(
            "config_production_manager",
...
#[allow(deprecated)]
/// Type alias for Productionmanagerconfigcanonical
pub type ProductionManagerConfigCanonical = crate::config::canonical_primary::domains::network::CanonicalNetworkConfig;

// Note: Keep using ProductionManagerConfig (the deprecated struct) for now.
...
mod tests {
    ...
    fn test_config_default() {
        let config = ProductionManagerConfig::default();
