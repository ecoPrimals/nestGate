...
impl CapabilityConfig {
    ...
    async fn build_self_knowledge() -> Result<SelfKnowledge> {
        ...
        let endpoints = Self::build_endpoints().await?;
    ...
    async fn build_endpoints() -> Result<Vec<ServiceEndpoint>> {
    ...
    pub async fn discover_capability(&self, capability: &str) -> Result<ServiceEndpoint> {
        ...
        if let Some(endpoint) = self.discover_from_environment(capability).await? {
    ...
    async fn discover_from_environment(&self, capability: &str) -> Result<Option<ServiceEndpoint>> {
    ...
    pub async fn announce(&self) -> Result<()> {
        ...
        for method in &self.discovery_config.methods {
            match self.announce_via_method(method).await {
    ...
    async fn announce_via_method(&self, method: &DiscoveryMethod) -> Result<()> {
        match method {
            DiscoveryMethod::Environment => {
