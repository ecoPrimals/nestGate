...
mod tests {
    ...
    use crate::network::native_async::types::ServiceEvent;
    use crate::Result;
    use uuid::Uuid;
    ...
    impl MockServiceDiscovery {
        ...
        async fn discover(&self, service_name: &str) -> Result<Vec<String>> {
            if let Some(endpoint) = self.services.get(service_name) {
    ...
    fn test_network_compile_time_specialization() {
        // Test compile-time network configurations
        assert_eq!(
            production::ProductionServiceDiscovery::max_services(),
            10000
        );
        assert_eq!(
            production::ProductionServiceDiscovery::discovery_timeout_seconds(),
            30
        );
        assert_eq!(
            production::ProductionServiceDiscovery::watch_buffer_size(),
            1000
        );

        assert_eq!(DevelopmentServiceDiscovery::max_services(), 1000);
        assert_eq!(DevelopmentServiceDiscovery::discovery_timeout_seconds(), 60);
        assert_eq!(DevelopmentServiceDiscovery::watch_buffer_size(), 100);

        // Validate these work at compile-time (no const needed for validation)
        let _prod_services: usize = production::ProductionServiceDiscovery::max_services();
        let _dev_services: usize = DevelopmentServiceDiscovery::max_services();

        println!("✅ Service discovery compile-time specialization working perfectly!");
        println!(
            "   Production services: {}, Development services: {}",
            _prod_services, _dev_services
        );
    }
}
