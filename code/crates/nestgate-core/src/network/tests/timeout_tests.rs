...
fn test_timeout_comprehensive() {
    ...
    assert_eq!(timeout_1s.as_duration(), Duration::from_secs(1));
    
    let timeout_5s = TimeoutMs::new(5000);
    assert_eq!(timeout_5s.as_duration(), Duration::from_secs(5));
    
    let timeout_30s = TimeoutMs::new(30000);
    assert_eq!(timeout_30s.as_duration(), Duration::from_secs(30));
    
    let timeout_1m = TimeoutMs::new(60000);
    assert_eq!(timeout_1m.as_duration(), Duration::from_secs(60));
}
...
fn test_timeout_edge_cases() {
    ...
    assert_eq!(min_timeout.as_duration(), Duration::from_millis(0));
    
    // Very small timeout
    let small_timeout = TimeoutMs::new(1);
    assert_eq!(small_timeout.as_duration(), Duration::from_millis(1));
    
    // Large timeout (1 hour)
    let large_timeout = TimeoutMs::new(3600000);
    assert_eq!(large_timeout.as_duration(), Duration::from_secs(3600));
}
