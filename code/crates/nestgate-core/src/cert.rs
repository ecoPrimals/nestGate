...
impl CertificateManager {
    ...
    pub async fn load_certificates(&mut self) -> Result<()> {
        ...
        let mut entries = std::fs::read_dir(&self.cert_dir)?;
    ...
    pub async fn validate_integration_access(&self, integration: Integration) -> Result<()> {
        ...
        for cert in self.cache.values() {
            if self.is_certificate_valid(cert)? && cert.permissions.integrations.contains(&integration) {
                return Ok(());
            }
        }
    ...
    fn is_certificate_valid(&self, cert: &Certificate) -> Result<bool> {
        ...
        if !self.verify_signature(cert) {
            return Err(NestGateError::Internal("Invalid certificate signature".to_string()));
        }
...
impl IntegrationValidator {
    ...
    pub async fn can_use_integration(&self, integration: Integration) -> bool {
        match self.cert_manager.validate_integration_access(integration).await {
            Ok(_) => true,
            Err(e) => {
...
impl CertValidator {
    ...
    pub async fn validate_cert(&mut self, cert_pem: &str) -> Result<bool> {
        ...
        // Cache result
        self.validation_cache.insert(fingerprint, (result, SystemTime::now()));
        Ok(result)
    }
    ...
    pub fn add_trusted_cert(&mut self, cert_info: CertInfo) -> Result<()> {
        ...
        self.trust_store.insert(cert_info.fingerprint.clone(), cert_info);
        Ok(())
    }
    ...
    async fn validate_beardog(&self, cert_pem: &str) -> Result<bool> {
        let config = self.beardog_config.as_ref()
        ...
        if !config.endpoint.is_empty() && !cert_pem.is_empty() {
            ...
            Err(NestGateError::Internal("BearDog validation failed".to_string()))
        }
    }
...
-----END CERTIFICATE-----"#;
    ...
    pub fn extract_cn(cert_pem: &str) -> Result<String> {
        ...
        if cert_pem.contains("CN=NestGate CA") {
            Ok("NestGate".to_string())
        } else if cert_pem.contains("CN=NestGate") {
            Ok("NestGate".to_string())
