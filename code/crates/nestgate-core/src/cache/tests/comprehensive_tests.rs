...
mod storage_tier_tests {
    ...
    fn test_storage_tier_access_time_magnitudes() {
        ...
        assert!(StorageTier::Cache.typical_access_time() < Duration::from_millis(1));

        // Archive should be multi-second
        assert!(StorageTier::Archive.typical_access_time() >= Duration::from_secs(1));
    }
