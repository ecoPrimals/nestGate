    ...
    pub fn to_nestgate_error(&self) -> nestgate_core::NestGateError {
        match self {
            ...
            ZfsError::ConfigError(msg) => {
                nestgate_core::NestGateError::Configuration(msg.clone())
            }
            _ => {
                nestgate_core::NestGateError::Internal(self.to_string())
            }
        }
    }
}
...
impl<T> ZfsErrorExt<T> for ZfsResult<T> {
    fn with_context(self, context: String) -> ZfsResult<T> {
        self.map_err(|e| match e {
            ZfsError::Internal(msg) => ZfsError::Internal(format!("{}: {}", context, msg)),
            other => other,
        })
    }
...
mod tests {
    ...
    fn test_error_severity() {
        ...
        let warning_error = ZfsError::PoolError(PoolError::NotFound { pool_name: "test".to_string() });
