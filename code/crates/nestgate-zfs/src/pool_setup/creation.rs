...
use super::{
    ...
    validation::{PoolSetupConfig, PoolTopology, ValidationResult},
};
...
impl PoolCreator {
    ...
    pub async fn create_pool_safe(&self, config: &PoolSetupConfig) -> CoreResult<PoolSetupResult> {
        ...
        if self.config.safety.require_confirmation {
            ...
            if self.config.safety.default_dry_run {
                info!("DRY RUN: Would create pool with configuration: {:#?}", config);
        ...
        match self.create_pool_internal(config).await {
            ...
            Err(e) => {
                ...
                // TODO: Implement rollback/cleanup
                Err(e)
            }
        }
    }
    ...
    async fn create_pool_internal(&self, config: &PoolSetupConfig) -> CoreResult<PoolSetupResult> {
        ...
        for (key, value) in &config.properties {
            if key.is_empty() || value.is_empty() {
                return Err(NestGateError::Internal(format!("Invalid property: {}={}", key, value)));
            }
    ...
    async fn create_single_tier(&self, pool_name: &str, tier: &StorageTier, tier_name: &str) -> CoreResult<()> {
        ...
        let tier_props = self.config.tier_config.tier_properties
            .get(tier_name)
            .cloned()
            .unwrap_or_else(|| {
        ...
        cmd.args(&["-o", &format!("secondarycache={}", tier_props.secondarycache)]);
    ...
    pub async fn destroy_pool_safe(&self, pool_name: &str, force: bool) -> CoreResult<()> {
        ...
        if !force && self.config.safety.require_confirmation {
            return Err(NestGateError::Internal(
                "Pool destruction requires explicit force flag when confirmation is required".to_string()
            ));
        }
        ...
        let output = cmd.output().await
            .map_err(|e| NestGateError::Internal(format!("Failed to execute zpool destroy: {}", e)))?;
