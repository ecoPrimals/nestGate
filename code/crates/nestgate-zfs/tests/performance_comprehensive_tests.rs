...
use nestgate_zfs::performance_engine::{
    ...
    SystemMemoryUsage, WorkloadPattern, ZfsBottleneck, ZfsBottleneckType, ZfsConfigurationContext,
    ZfsDatasetMetrics, ZfsPerformanceMetrics, ZfsPoolMetrics, ZfsTuningRecommendation,
    ZfsTuningResult,
};
use nestgate_zfs::{ZfsDatasetManager, ZfsPoolManager};
...
mod test_utils {
    ...
    pub fn create_workload_patterns() -> HashMap<String, WorkloadPattern> {
        ...
        patterns.insert(
            "file_server_workload".to_string(),
            WorkloadPattern {
...
mod performance_metrics_comprehensive_tests {
    ...
    fn test_comprehensive_metrics_structure() {
        ...
        assert_eq!(
            memory.used_memory + memory.available_memory,
            memory.total_memory
        );
...
mod bottleneck_detection_comprehensive_tests {
    ...
    fn test_bottleneck_scenario_creation() {
        ...
        assert!(types
            .iter()
            .any(|t| matches!(t, ZfsBottleneckType::HighLatency)));
        assert!(types
            .iter()
            .any(|t| matches!(t, ZfsBottleneckType::LowCacheHitRatio)));
        assert!(types
            .iter()
            .any(|t| matches!(t, ZfsBottleneckType::HighFragmentation)));
        assert!(types
            .iter()
            .any(|t| matches!(t, ZfsBottleneckType::RecordSizeMismatch)));
    }
    ...
    fn test_bottleneck_severity_classification() {
        ...
        let high_severity: Vec<_> = bottlenecks
            .iter()
            .filter(|b| matches!(b.severity, BottleneckSeverity::High))
            .collect();
        let medium_severity: Vec<_> = bottlenecks
            .iter()
            .filter(|b| matches!(b.severity, BottleneckSeverity::Medium))
            .collect();
        let low_severity: Vec<_> = bottlenecks
            .iter()
    ...
    fn test_optimization_result_creation() {
        ...
        let optimizations = vec![
            ...
            AppliedOptimization {
                ...
                parameters_changed: vec![("recordsize".to_string(), "1M".to_string())],
        ...
        result.performance_improvement_estimate =
            "Combined 35% performance improvement".to_string();
        result.warnings = vec![
...
mod alert_handling_comprehensive_tests {
    ...
    fn test_alert_creation_and_classification() {
        ...
        assert!(alert_types
            .iter()
            .any(|t| matches!(t, AlertType::HighLatency)));
        assert!(alert_types
            .iter()
            .any(|t| matches!(t, AlertType::LowThroughput)));
        assert!(alert_types
            .iter()
            .any(|t| matches!(t, AlertType::CacheInefficiency)));
        assert!(alert_types
            .iter()
            .any(|t| matches!(t, AlertType::FragmentationHigh)));

        // Verify severity distribution
        let severities: Vec<_> = alerts.iter().map(|a| &a.severity).collect();
        assert!(severities
            .iter()
            .any(|s| matches!(s, AlertSeverity::Critical)));
        assert!(severities.iter().any(|s| matches!(s, AlertSeverity::High)));
        assert!(severities
            .iter()
            .any(|s| matches!(s, AlertSeverity::Warning)));
    ...
    fn test_alert_response_generation() {
        ...
        let response = AlertResponse {
            ...
            root_cause_analysis: Some(
                "High latency likely caused by storage device saturation or pool fragmentation"
                    .to_string(),
            ),
        };
        ...
        assert!(response
            .immediate_actions
            .iter()
            .any(|a| a.contains("pool health")));
        assert!(response
            .immediate_actions
            .iter()
            .any(|a| a.contains("disk health")));
        assert!(response
            .long_term_recommendations
            .iter()
            .any(|r| r.contains("defragmentation")));
        assert!(response
            .long_term_recommendations
            .iter()
            .any(|r| r.contains("caching")));
    }
}
...
mod zfs_tuning_comprehensive_tests {
    ...
    fn test_tuning_recommendations() {
        let recommendations = vec![
            ZfsTuningRecommendation {
                ...
                expected_improvement: "40% throughput improvement for large file operations"
                    .to_string(),
            },
            ZfsTuningRecommendation {
            ...
            ZfsTuningRecommendation {
                ...
                expected_improvement: "Better ARC utilization for metadata-heavy workloads"
                    .to_string(),
            },
        ];
    ...
    fn test_tuning_result_comprehensive() {
        let result = ZfsTuningResult {
            ...
            expected_improvement: "Combined 60% performance improvement with 30% space savings"
                .to_string(),
            warnings: vec![
        ...
        for bottleneck in &bottlenecks {
            ...
            match bottleneck.severity {
                BottleneckSeverity::Critical | BottleneckSeverity::High => {
                    ...
                    assert!(matches!(
                        optimization_type,
                        OptimizationType::LatencyOptimization | OptimizationType::CacheOptimization
                    ));
                }
