...
use crate::error::{NestGateUnifiedError, Result};
use crate::constants::unified::performance;
use std::arch::x86_64::*;
use std::mem;
...
mod tests {
    ...
    fn test_simd_engine_creation() {
        ...
        assert!(engine.vector_width() >= 16, "Vector width should be at least 16 bytes");
        assert!(engine.alignment() >= engine.vector_width(), "Alignment should match vector width");
        
        println!("SIMD Engine: {} bytes vector, {} bytes alignment", 
                 engine.vector_width(), engine.alignment());
    }
    ...
    fn test_batch_processor() {
        ...
        assert_eq!(processed.len(), data.len(), "Output length should match input");
    ...
    fn test_simd_metrics() {
        ...
        assert_eq!(metrics.speedup_factor, 4.5, "Speedup should match set value");
        assert!(metrics.memory_bandwidth_gbps > 0.0, "Bandwidth should be positive");
        
        let report = metrics.report();
        assert!(report.contains("test_operation"), "Report should contain operation name");
        assert!(report.contains("4.50x"), "Report should contain speedup factor");
    }
    ...
    fn test_initialization() {
        ...
        assert!(engine.capabilities().sse2, "Should have at least SSE2 support");
    }
} 