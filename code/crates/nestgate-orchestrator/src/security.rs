...
impl SecurityManager {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        tracing::info!("Rate limiting enabled: {}", self.config.rate_limiting.enabled);
    ...
    pub fn check_rate_limit(&self, client_ip: &str) -> Result<()> {
        ...
        let rate_limit = rate_limits.entry(client_ip.to_string()).or_insert(RateLimit {
    ...
    pub fn validate_api_key(&self, key: &str) -> bool {
        self.config.api_keys.iter().any(|api_key| api_key.key == key)
    }
