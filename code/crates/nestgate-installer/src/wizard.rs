...
impl InstallationWizard {
    ...
    fn confirm_installation(&self) -> Result<()> {
        ...
        println!("Service Mode: {}", if self.config.service_mode { "Yes" } else { "No" });
        println!("ZFS Support: {}", if self.config.enable_zfs { "Yes" } else { "No" });
        println!("AI Features: {}", if self.config.ai_enabled { "Yes" } else { "No" });
        ...
        println!("Add to PATH: {}", if self.config.add_to_path { "Yes" } else { "No" });
        println!("Desktop Shortcut: {}", if self.config.create_desktop_shortcut { "Yes" } else { "No" });
