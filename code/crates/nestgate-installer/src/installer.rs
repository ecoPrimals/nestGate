...
impl NestGateInstaller {
    ...
    pub async fn install(&mut self, force: bool, as_service: bool, _skip_zfs: bool, yes: bool) -> Result<()> {
        ...
        let archive_path = self.downloader.download_release(&version, &temp_dir).await?;
        self.downloader.extract_archive(&archive_path, &install_path)?;
        ...
        let install_info = InstallationInfo {
            ...
            features: vec![
                if config.enable_zfs { "zfs".to_string() } else { "".to_string() },
                if config.ai_enabled { "ai".to_string() } else { "".to_string() },
        ...
        println!();
        println!("{}", green.apply_to("✅ NestGate installation completed successfully!"));
        println!();
        println!("Next steps:");
        ...
        if install_info.service_installed {
            ...
            println!("  • Start NestGate: {}", install_path.join("bin").join("nestgate").display());
        }
        
        Ok(())
    }
    
    pub async fn uninstall(&mut self, remove_config: bool, remove_data: bool, yes: bool) -> Result<()> {
        ...
        let installation_info = self.get_installation_info()
        ...
        if !yes {
            let message = format!(
                ...
                if remove_config { " (including config)" } else { "" },
        ...
        if installation_info.install_path.exists() {
            ...
            info!("Removed installation directory: {}", installation_info.install_path.display());
        }
        
        // Remove configuration if requested
        if remove_config && installation_info.config_path.exists() {
            if installation_info.config_path != installation_info.install_path.join("etc").join("nestgate.toml") {
                fs::remove_dir_all(&installation_info.config_path)?;
                info!("Removed configuration: {}", installation_info.config_path.display());
            }
        }
        ...
        if remove_data && installation_info.data_path.exists() {
            ...
            info!("Removed data directory: {}", installation_info.data_path.display());
        }
        ...
        println!("{}", yellow.apply_to("✅ NestGate uninstalled successfully"));
    ...
    pub async fn update(&mut self, version: Option<String>, yes: bool) -> Result<()> {
        ...
        if !yes && !Confirm::new()
            .with_prompt(&format!("Update NestGate from {} to {}?", installation_info.version, target_version))
            .interact()? 
        {
        ...
        let archive_path = self.downloader.download_release(&target_version, &temp_dir).await?;
        self.downloader.extract_archive(&archive_path, &installation_info.install_path)?;
        
        // Restore configuration
        let old_config = backup_path.join("etc").join("nestgate.toml");
        let new_config = installation_info.install_path.join("etc").join("nestgate.toml");
        ...
        // Verify installation
        self.downloader.verify_installation(&updated_info.install_path)?;
        
        // Cleanup
        fs::remove_dir_all(&backup_path)?;
        fs::remove_dir_all(&temp_dir)?;
        
        println!("✅ Update completed successfully to version {}", updated_info.version);
    ...
    pub async fn run_configuration_wizard(&mut self) -> Result<()> {
        ...
        println!("✅ Configuration updated: {}", installation_info.config_path.display());
    ...
    pub async fn doctor(&mut self) -> Result<()> {
        ...
        match self.get_installation_info() {
            Ok(info) => {
                ...
                println!("   Service: {}", if info.service_installed { "Yes" } else { "No" });
            }
            Err(_) => {
        ...
        println!("{} Platform: {}-{}", green.apply_to("✓"), self.platform.os, self.platform.arch);
        if self.platform.service_install_supported() {
            println!("{} Service installation supported", green.apply_to("✓"));
        } else {
            println!("{} Service installation not supported", yellow.apply_to("⚠"));
        }
    ...
    fn get_installation_info(&self) -> Result<InstallationInfo> {
        ...
        let info_json = fs::read_to_string(&info_path)
            .context("Installation info not found")?;
        let info: InstallationInfo = serde_json::from_str(&info_json)
            .context("Invalid installation info format")?;
        Ok(info)
    }
