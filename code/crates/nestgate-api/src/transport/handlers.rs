...
impl<S: StorageBackend + 'static> NestGateRpcHandler<S> {
    ...
    pub(crate) fn handle_ping(&self, _params: Value) -> Result<Value> {
    ...
    pub(crate) fn handle_status(&self, _params: Value) -> Result<Value> {
    ...
    pub(crate) fn handle_system_info(&self, _params: Value) -> Result<Value> {
        let primal = self_primal_name();
        Ok(serde_json::json!({
            "primal": primal,
            "version": env!("CARGO_PKG_VERSION"),
    ...
    pub(crate) fn handle_lifecycle_status(&self) -> Result<Value> {
        Ok(serde_json::json!({
            "status": "running",
