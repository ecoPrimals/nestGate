...
pub fn create_stub_compute_resources() -> ComputeResources {
    ComputeResources {
        // ecoBin v3.0: stubs only; production uses `/proc` via `hardware_tuning::linux_proc` (not sysinfo).
        available_cpu: 16,
...
mod tests {
    ...
    fn test_profile_names_are_lowercase() {
        ...
        assert_eq!(profile.memory_profile.to_lowercase(), profile.memory_profile);
    }
