...
//!
//! JSON endpoints that share `/proc` discovery with `RealHardwareTuningHandler`
//! via the `linux_proc` module (no `sysinfo`).
...
fn collect_live_metrics(handler: &RealHardwareTuningHandler) -> Result<LiveHardwareMetrics> {
    handler.get_live_hardware_metrics()
}

/// Expose current compute resources (from `/proc`).
pub fn get_hardware_info() -> Result<Json<ComputeResources>> {
...
pub fn get_system_profile() -> Result<Json<SystemProfile>> {
    let handler = RealHardwareTuningHandler::new();
    let profile = handler.get_derived_system_profile()?;
    Ok(Json(profile))
}

/// Optimize hardware performance (observational; no privileged tuning).
pub fn optimize_hardware_performance() -> Result<Json<TuningResult>> {
    let handler = RealHardwareTuningHandler::new();
    let tuning_result = handler.auto_tune()?;
...
pub fn get_compute_resources() -> Result<Json<ComputeResources>> {
    get_hardware_info()
}
...
pub fn run_hardware_benchmark() -> Result<Json<BenchmarkResult>> {
    let handler = RealHardwareTuningHandler::new();
    let start_time = std::time::Instant::now();
    let resources = linux_proc::compute_resources_from_proc()?;
    let metrics = collect_live_metrics(&handler)?;
...
pub fn start_hardware_tuning_session() -> Result<Json<LiveHardwareTuningSession>> {
    let handler = RealHardwareTuningHandler::new();
    let metrics = collect_live_metrics(&handler)?;
    let resources = linux_proc::compute_resources_from_proc()?;
...
pub fn request_compute_resources(
    ...
    let handler = RealHardwareTuningHandler::new();
    let allocation = handler.request_compute_resources(&request)?;
    Ok(Json(allocation))
}
