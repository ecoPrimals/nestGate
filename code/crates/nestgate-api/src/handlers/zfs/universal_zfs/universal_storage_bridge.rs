...
impl UniversalStorageBridge {
                ...
                if Self::should_include_filesystem(source, fstype, mount_point) {
                    ...
                    pools.push(PoolInfo {
                        ...
                        properties: {
                            ...
                            props.insert("filesystem_type".into(), fstype.to_string());
                            props.insert("mount_point".into(), mount_point.to_string());
                            props
                        },
                    });
                }
    ...
    fn parse_size_to_bytes(size_str: &str) -> UniversalZfsResult<u64> {
        ...
        let number: f64 = number_part
            .parse()
            .map_err(|_| UniversalZfsError::internal("Failed to parse size value"))?;
    ...
    fn create_zfs_dataset(&self, config: &DatasetConfig) -> UniversalZfsResult<DatasetInfo> {
        ...
        let output = std::process::Command::new("zfs")
            ...
            .output()
            .map_err(|_e| UniversalZfsError::internal("Failed to execute command"))?;
