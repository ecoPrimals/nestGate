...
description = "Content-addressed storage primal v0.5.0 — 1,630+ tests, 18 crates, 20 capability domains, 4 transport surfaces, Session 134 (dead module purge: auth_production + zero_cost_api_handlers + models.rs removed 3,573 LOC; rustix 0.38→1.1 unified; content.fetch streaming fix; federation blob extraction; HTTP client dedup; fsmonitor/middleware quarantined; fabricated metrics purge; CAS federation streaming; dep evolution; stub evolution; G3/G4; BTSP; procfs consolidated; vendor eliminated; all internal crypto → BLAKE3; prod unwrap: 0/10 expect; deep debt sweep), CI-DIV-03, NESTGATE-ANDROID-01, STARTUP-NG-01, riboCipher, BTSP auth"
date = 2026-08-03
...
- **Session 134 Dead module purge + dep unification**: `auth_production` (8 files), `zero_cost_api_handlers` (7 files), `models.rs` purged — 3,573 LOC compiled but never routed. `rustix` 0.38→1.1 unified with tempfile (eliminated `rustix`+`linux-raw-sys` duplicates). `nestgate-fsmonitor` + `nestgate-middleware` quarantined from workspace (unwired, zero dependents). Commented-out code removed from `discovery.rs` and `production.rs`.
- **Session 133 content.fetch streaming fix + federation extraction**: Fixed ureq 3.x compile error (`body.as_reader()`); rewrote to stream HTTP response to `.part` file while BLAKE3 hashing (O(1) memory); removed `unwrap()` from production path; propagated meta write errors. Extracted federation blob transfer helpers (`content_federation_handlers.rs` 802→420 lines). Deduplicated `validate_fetch_url` + ureq agent builder into shared `storage_paths.rs`.
- **Session 130 CAS federation streaming + size guard + remote decoupling** (Wave 155n): Federation `content.replicate` / `content.replicate.pull` now use chunked streaming (4 MiB chunks via `content.store_stream` / `content.retrieve_stream`) for blobs > 16 MiB; `content.get` returns `use_streaming: true` for blobs > 64 MiB instead of loading full payload; `pull_blob_from_remote` probes size via `content.exists` and transparently switches to streaming; hardcoded `"forgejo"` remote → `NESTGATE_PREFERRED_REMOTE` env var; tests extracted to sibling file (production 980→797 lines); atomic writes for pull via temp+rename
- **Session 129 Dep evolution + stub evolution + fabricated fallback cleanup** (Wave 155b): `serde/rc` + `tower-http/fs` features removed (unused); stale lock entries pruned; `should_transition_to_stage` evolved from always-`false` stub to live lifecycle logic (age + access → natural stage comparison via `stage_ordinal()`); `get_all_pool_metrics` evolved from `NotImplemented` to live `zpool list` collection; `get_cache_metrics` evolved to live ARC/L2ARC snapshot (new `arc_and_l2arc_sizes()` helper); disk I/O fallback magic numbers (1GB/512MB/10k ops) → honest zeroes; ARC fallback fabricated percentages (90/70/85/65%) → 0.0; stale `#![expect(clippy::unnecessary_wraps)]` removed
- **Session 128 G3 BTSP peer wiring + G4 NTFS CAS safety + deep debt** (Wave 155b): 3 peer connection sites upgraded to `connect_btsp_aware` (crypto delegate, coordinator announce, atomic health); `OutboundEndpoint::connect_jsonrpc()` added; NTFS-safe CAS — `validate_path_segment()` rejects reserved chars/trailing dot/max length, strict lowercase BLAKE3 validation, atomic `content.put` via temp+rename, Windows storage marker guard; biomeOS identifiers renamed (`BiomeOSDirectory` → `EcosystemDirectory`, `socket_parent_is_biomeos_standard_dir` → `socket_parent_is_ecosystem_standard_dir`)
