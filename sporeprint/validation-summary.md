...
description = "Content-addressed storage primal v0.5.0 — 1,710 tests, 20 crates, 20 capability domains, 4 transport surfaces, Wave 155b (dep evolution: serde/rc + tower-http/fs trimmed; stub evolution: should_transition_to_stage → live lifecycle logic, get_all_pool_metrics + get_cache_metrics → live ZFS data; fabricated fallback cleanup: disk I/O + ARC zeros; G3: BTSP peer wiring + G4: NTFS CAS safety; BTSP ClientHello shipped; procfs consolidated; vendor eliminated; all internal crypto → BLAKE3; prod unwrap: 0/10 expect; deep debt sweep), CI-DIV-03, NESTGATE-ANDROID-01, STARTUP-NG-01, riboCipher, BTSP auth"
date = 2026-07-27
