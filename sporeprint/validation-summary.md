...
description = "Content-addressed storage primal v0.5.0 — 13,095+ tests, 20 crates, 20 capability domains, 4 transport surfaces, Session 131 (fabricated metrics purge: dashboard → live statvfs + /proc, hw-tuning delta computation, ZFS AI field sunset, tokio-util dep pruned, stale demo/placeholder comments purged; CAS federation streaming; dep evolution; stub evolution; G3/G4; BTSP; procfs consolidated; vendor eliminated; all internal crypto → BLAKE3; prod unwrap: 0/10 expect; deep debt sweep), CI-DIV-03, NESTGATE-ANDROID-01, STARTUP-NG-01, riboCipher, BTSP auth"
date = 2026-08-03
...
- **13,095+ tests**, **0 failures** (serial and parallel), 0 clippy warnings
- **Session 131 Deep debt — fabricated metrics purge + dep hygiene**: Dashboard `get_overview()` evolved from 7 fabricated values (disk 45%, network 1M, connections 25, response 150ms, error 0.1%, trend series, capacity forecast) to live `/proc`+`statvfs` data with honest zeroes; `start_collection()` now writes to metrics cache; `estimated_power_increase` / `performance_improvement` computed from before/after delta (3 sites); `AiIntegrationStatus` → `None` (sunset); `tokio-util` direct dep removed from `nestgate-rpc` (zero `tokio_util::` in code), `codec` feature dropped from workspace; stale "demo"/"placeholder" comments cleaned across 10+ production files; 4 test assertions updated for honest values
