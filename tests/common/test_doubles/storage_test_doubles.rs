...
impl MockStorageForTesting {
    ...
    pub async fn fake_create_pool(&self, name: &str) -> Result<(), TestStorageError> {
        self.test_double.simulate_operation(&format!("create_pool:{}", name)).await?;
    ...
    pub async fn fake_create_dataset(&self, pool: &str, dataset: &str, _tier: StorageTier) -> Result<(), TestStorageError> {
        self.test_double.simulate_operation(&format!("create_dataset:{}:{}", pool, dataset)).await?;
        
        if let Ok(mut data) = self.fake_data.lock() {
            data.insert(format!("dataset:{}:{}", pool, dataset), "created".to_string());
        }
