...
use nestgate_core::{
    crypto_locks::{
        ExternalBoundaryGuardian, ExternalLockType, 
        CryptographicProof, ExtractionRestrictions, CopyleftRequirements,
        AccessDecision,
    },
...
async fn test_beardog_exclusive_key_management() -> Result<()> {
    ...
    let crypto_proof = CryptographicProof::new_with_beardog(
        &cert_validator,
        "test-operation",
        "test-destination",
    ).await?;
    ...
    let is_valid = crypto_proof.validate_with_beardog(
        &cert_validator,
        "test-operation",
        "test-destination",
    ).await?;
    ...
    let access_decision = guardian.check_external_boundary(
        "nestgate-core",
        "internal-system",
        "read",
    ).await?;
    ...
    let external_access = guardian.check_external_boundary(
        "nestgate-core",
        "https://external-api.com",
        "extract",
    ).await?;
    
    assert!(matches!(external_access, AccessDecision::RequireLock { .. }));
...
async fn test_beardog_cryptographic_proof() -> Result<()> {
    ...
    assert!(proof.ecosystem_fingerprint.starts_with("nestgate-ecosystem-"));
    
    println!("✅ BearDog proof created with ecosystem fingerprint");
    
    // Test proof validation
    let is_valid = proof.validate_with_beardog(
        &cert_validator,
        "backup",
        "https://s3.amazonaws.com/bucket",
    ).await?;
    ...
    let is_invalid = proof.validate_with_beardog(
        &cert_validator,
        "clone",
        "https://s3.amazonaws.com/bucket",
    ).await?;
    
    assert!(!is_invalid, "Proof should be invalid for different operation");
...
async fn test_external_company_sovereign_lock() -> Result<()> {
    ...
    let lock_id = guardian.create_sovereign_beardog_lock(
        "bigtech-corp.com",
...
async fn test_beardog_key_ecosystem_sovereignty() -> Result<()> {
    ...
    assert!(key_id.starts_with("beardog-key-"), "Key ID should be BearDog-generated");
    
    // Test 2: Verify signatures are BearDog-generated
    let signature = validator.sign_data("test-data").await?;
    assert!(signature.starts_with("beardog-sig-"), "Signature should be BearDog-generated");
...
async fn test_comprehensive_beardog_integration() -> Result<()> {
    ...
    let internal_rust = guardian.check_external_boundary(
    ...
    let internal_primal = guardian.check_external_boundary(
        "ecoprimal:storage",
        "primal:analytics",
        "process"
    ).await?;
    ...
    let aws_access = guardian.check_external_boundary(
        "nestgate-core",
    ...
    let github_access = guardian.check_external_boundary(
        "nestgate-core",
    ...
    let stripe_access = guardian.check_external_boundary(
        "nestgate-core",
        "https://api.stripe.com",
        "payment"
    ).await?;
...
async fn test_beardog_sovereignty_model() -> Result<()> {
    ...
    let ecosystem_comm = guardian.check_external_boundary(
        "nestgate-zfs",
        "nestgate-automation",
        "optimize"
    ).await?;
    ...
    let external_request = guardian.check_external_boundary(
        "nestgate-core",
        "https://bigtech-corp.com/api",
        "extract"
    ).await?;
    
    assert!(matches!(external_request, AccessDecision::RequireLock { .. }));
    println!("✅ External company: Must use BearDog crypto lock");
    
    // Scenario 3: Create sovereign lock for external company
    let sovereign_lock = guardian.create_sovereign_beardog_lock(
        "bigtech-corp.com",
        vec!["read".to_string(), "analyze".to_string()],
        Some(30), // 30 days
    ).await?;
    ...
    let ai_access = guardian.check_external_boundary(
        "squirrel:ai-api",
