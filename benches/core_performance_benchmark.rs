...
fn benchmark_error_context(c: &mut Criterion) {
    ...
    c.bench_function("error_context_addition", |b| {
        b.iter(|| {
            ...
            context_map.insert("component".to_string(), "performance_validation".to_string());
...
fn benchmark_string_processing(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000].iter() {
        group.bench_with_input(BenchmarkId::new("unified_string_ops", size), size, |b, &size| {
            b.iter(|| {
        ...
        group.bench_with_input(BenchmarkId::new("fragmented_string_ops", size), size, |b, &size| {
            b.iter(|| {
